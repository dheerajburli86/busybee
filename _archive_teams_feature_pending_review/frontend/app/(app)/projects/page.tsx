...
import { COLOR_SWATCHES, Milestone } from "@/components/tasks/types";
              ...
              <div key={project.id} className="p-4 sm:p-6 bg-slate-800 rounded border border-slate-700">
                ...
                {openProject === project.id && (
                  <div className="mt-4 pt-4 border-t border-slate-700 space-y-4">
                    {/* SOW #17: this project's milestones, each with its own date. */}
                    <MilestonesPanel project={project} inputCls={inputCls} onError={setError} />
                    {project.can_manage && (
...
/**
 * SOW #17: the milestones of one project. Unlike the old fixed dropdown
 * ("Phase 1", "Launch") these are the project's own, each with a date, so the
 * Gantt can mark them and tasks can be grouped under something meaningful.
 *
 * The milestones table arrived after the rest of the app: on a database that
 * hasn't had the migration run the fetch fails and the panel stays hidden
...
function MilestonesPanel({
  ...
  const [editing, setEditing] = useState<{ id: string; name: string; due: string } | null>(null);
  const can = !!project.can_manage;

  const load = async () => {
    try {
      const r = await fetch(`/api/milestones?project_id=${project.id}`, { cache: "no-store" });
      if (!r.ok) {
        setReady(false);
        return;
      }
  ...
  const call = async (method: "POST" | "PUT" | "DELETE", body: any) => {
    try {
      await sendJSON("/api/milestones", method, body);
      await load();
      return true;
    } catch (e: any) {
      onError(e.message);
  ...
  const add = async () => {
    ...
    if (await call("POST", { project_id: project.id, name: draft.name.trim(), due_date: draft.due || null })) {
      setDraft({ name: "", due: "" });
    }
  };
  ...
  const saveEdit = async () => {
    ...
    if (await call("PUT", { id: editing.id, name: editing.name.trim(), due_date: editing.due || null })) {
      setEditing(null);
    }
  };
  ...
  return (
    <div className="space-y-2">
      ...
      {milestones.length === 0 && <p className="text-xs text-slate-500">No milestones yet.</p>}
      <div className="space-y-1">
        {milestones.map((m) => {
          const late = m.due_date && new Date(m.due_date) < new Date();
          return (
            <div key={m.id} className="flex flex-wrap items-center justify-between gap-2 bg-slate-900 rounded px-3 py-2">
              {editing?.id === m.id ? (
                <div className="flex flex-wrap gap-2 flex-1">
                  ...
                  />
                  <button onClick={saveEdit} className="px-3 py-1 bg-blue-600 hover:bg-blue-500 rounded text-xs">Save</button>
                ...
                <>
                  <div className="min-w-0">
                    ...
                    <p className="text-xs text-slate-500">
                      {m.due_date ? (
                        <span className={late ? "text-amber-400" : ""}>{new Date(m.due_date).toLocaleDateString()}</span>
                      ) : (
                        "No date"
                      )}
                      {` · ${m.task_count || 0} ${m.task_count === 1 ? "task" : "tasks"}`}
                    </p>
                  </div>
                  {can && (
                    <div className="flex gap-2">
                      <button
                        onClick={() => setEditing({ id: m.id, name: m.name, due: m.due_date ? m.due_date.slice(0, 10) : "" })}
                      ...
                      <button
                        onClick={() => {
                          if (confirm(`Remove the milestone "${m.name}"? Its tasks stay, they just stop pointing at it.`)) call("DELETE", { id: m.id });
                        }}
      ...
      {can && (
        <div className="flex flex-wrap gap-2">
          <input
            ...
            className={`${inputCls} text-xs py-1 flex-1 min-w-36`}
          />
          <input
            type="date"
            value={draft.due}
            ...
            aria-label="Milestone date"
          />
          <button onClick={add} disabled={!draft.name.trim()} className="px-3 py-1 bg-slate-700 hover:bg-slate-600 disabled:opacity-50 rounded text-xs">
            Add
          </button>
        </div>
      )}
