...
export default function DashboardPage() {
  ...
  // SOW #41/#29: the teams and groups work can be handed to. Empty when none
  // have been set up, or when the database hasn't had the migration run - the
  // picker and filter simply don't appear.
  ...
  const [form, setForm] = useState({
    ...
    section: "",
    assignee: "",
    // SOW #41: hand the work to a whole team or an ad-hoc group.
    team: "",
    color: "",
  });
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // #41: optional extra - never let a missing teams table break the page.
  ...
  const createTask = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setForm({ ...form, title: "", description: "", start: "", due: "", assignee: "", team: "", color: "" });
  ...
  const filtered = useMemo(() => {
    return source
      ...
      .filter((t) => (teamF ? (teamF === "none" ? !t.team_id : t.team_id === teamF) : true))
      .sort((a, b) => {
        if (sortBy === "due_date") {
  ...
  }, [source, q, mine, projectF, statusF, priorityF, assigneeF, teamF, sortBy, lookups]);

  const activeFilters = [statusF, priorityF, assigneeF, teamF, projectF, mine ? "mine" : ""].filter(Boolean).length;
    ...
    <div className="max-w-7xl mx-auto p-3 sm:p-6">
      ...
      <form onSubmit={createTask} className="bg-slate-800 p-3 sm:p-4 rounded border border-slate-700 mb-6 space-y-3">
        ...
        <div className="flex flex-wrap gap-2 items-end">
          ...
          {teams.length > 0 && (
            // SOW #41: or give it to a whole team / an ad-hoc group.
            <label className="flex flex-col text-xs text-slate-400 gap-1">
              Team
              <select value={form.team} onChange={(e) => setForm({ ...form, team: e.target.value })} className={inputCls}>
                ...
                {teams.some((t) => t.kind !== "group") && (
                  <optgroup label="Teams">
                    {teams.filter((t) => t.kind !== "group").map((t) => <option key={t.id} value={t.id}>{t.name}</option>)}
                  </optgroup>
                )}
                {teams.some((t) => t.kind === "group") && (
      ...
      {showFilters && (
        <div className="flex flex-wrap gap-2 mb-4 bg-slate-800 border border-slate-700 rounded p-3">
          ...
          {teams.length > 0 && (
            <select value={teamF} onChange={(e) => setTeamF(e.target.value)} className={inputCls} aria-label="Team">
          ...
          {activeFilters > 0 && (
            <button onClick={() => { setStatusF(""); setPriorityF(""); setAssigneeF(""); setTeamF(""); setProjectF(""); setMine(false); }} className="text-sm text-slate-400 hover:text-white px-2">
              Clear
            </button>
          )}
        </div>
            ...
            return (
              ...
              >
                ...
                <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-slate-500 mb-2">
                  ...
                  {/* SOW #29: the team it was given to, alongside the person. */}
                  {t.team_id && teams.some((x) => x.id === t.team_id) && (
                    <span>👥 {teams.find((x) => x.id === t.team_id)?.name}</span>
                  )}
