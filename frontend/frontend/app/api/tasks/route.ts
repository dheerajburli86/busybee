...
const MANAGE_FIELDS = [
  ...
  "start_date",
  "assigned_to",
  // SOW #29: a task can be given to a team or group as well as to a person.
  "team_id",
  "milestone",
  // SOW #17: the real, dated milestone. `milestone` above is the legacy text.
  "milestone_id",
  "archived_at",
  "task_manager_id",
  "key_result_id",
...
export async function GET(request: NextRequest) {
  try {
    ...
    const [visible, myProjects, myTeams, reminders] = await Promise.all([
      visibleTasks(supabase, user.id, tasks || []),
      userProjectIds(supabase, user.id),
      userTeamIds(supabase, user.id),
    ...
    return NextResponse.json({
      tasks: visible.map((t: any) => ({
        ...
        for_me: isForMe(t, user.id, myProjects, myTeams),
      })),
    });
  } catch (error: any) {
...
export async function POST(request: NextRequest) {
  try {
    ...
    const {
      ...
      remind_at,
      color,
      // SOW #29/#41: hand the work to a team or group as well as (or instead
      // of) one person. SOW #17: the dated milestone it belongs to.
      team_id,
      milestone_id,
    } = body;
    ...
    if (!personal) {
      const problem = await notOnDesk(supabase, ctx.desk_id, { assigned_to, project_id, team_id, milestone_id });
      if (problem) return NextResponse.json({ error: problem }, { status: 400 });
    }

    // SOW #17: a milestone belongs to one project, so it has to be one of
    // this task's own project's milestones.
    if (milestone_id) {
      const { data: ms } = await supabase
        ...
        .eq("id", milestone_id)
        .maybeSingle();
      if (!ms || ms.project_id !== ctx.project_id) {
        return NextResponse.json({ error: "That milestone isn't part of the task's project" }, { status: 400 });
      }
    }
    ...
    const { data: task, error } = await supabase
      ...
      .insert({
        ...
        created_by: user.id,
        color: color || null,
        // Only sent when the caller actually picked one, so a database that
        // hasn't had the teams/milestones migration run still accepts inserts.
        ...(!personal && team_id ? { team_id } : {}),
    ...
    // Tell whoever the work was given to - a person, a team, and/or the
    // whole project (SOW #2, #29).
    const recipients = new Set<string>();
    if (task.assigned_to) recipients.add(task.assigned_to);
    const [projectPeople, teamPeople] = await Promise.all([
      projectMemberIds(supabase, task.project_id),
      teamMemberIds(supabase, task.team_id),
    ]);
    projectPeople.forEach((id) => recipients.add(id));
    teamPeople.forEach((id) => recipients.add(id));
    ...
    const [myProjects, myTeams] = await Promise.all([
      userProjectIds(supabase, user.id),
      userTeamIds(supabase, user.id),
    ]);
    return NextResponse.json({ task: { ...task, subtask_count: 0, for_me: isForMe(task, user.id, myProjects, myTeams) } });
  } catch (error: any) {
...
export async function PUT(request: NextRequest) {
  try {
    ...
    if ("remind_at" in patch) {
      ...
      if (Object.keys(patch).length === 0) {
        const [{ count }, myProjects, myTeams] = await Promise.all([
          supabase.from("subtasks").select("id", { count: "exact", head: true }).eq("task_id", id),
          userProjectIds(supabase, user.id),
          userTeamIds(supabase, user.id),
        ]);
        return NextResponse.json({
          task: { ...before, remind_at: myReminder ?? null, subtask_count: count || 0, for_me: isForMe(before, user.id, myProjects, myTeams) },
        });
      }
    }
    if (patch.project_id) {
    ...
    // SOW #17: milestones belong to one project, so a task moved to another
    // project can't keep pointing at the old project's milestone. Clear it
    // unless the same request is setting a new one.
    //
    // Only when the task actually has one: on a database that hasn't had the
    // milestones migration run there is no such column, and naming it in the
    // update would fail the whole request - moving a task between projects
    // has to keep working there exactly as it did before.
    if (
      patch.project_id &&
      patch.project_id !== before.project_id &&
      !("milestone_id" in patch) &&
      before.milestone_id
    ) {
      patch.milestone_id = null;
    }
    // A milestone belongs to one project, so it must be a milestone of the
    // project the task ends up in - not merely one on the same desk.
    if (patch.milestone_id) {
      const { data: ms } = await supabase
        .from("milestones")
        .select("project_id")
        .eq("id", patch.milestone_id)
        .maybeSingle();
      if (!ms || ms.project_id !== projectAfter) {
        return NextResponse.json({ error: "That milestone isn't part of the task's project" }, { status: 400 });
      }
    }
    ...
    // Assignment: a person, a newly chosen team (SOW #29), or everyone in a
    ...
    if (changes.team_id && task.team_id) {
      (await teamMemberIds(supabase, task.team_id)).forEach((x) => newlyAssigned.add(x));
    }
    ...
    // SOW #29: and so does the team it was taken from, minus anyone who is
    // in the new team as well - they are being told it's theirs now.
    if (changes.team_id && before.team_id && before.team_id !== task.team_id) {
      const stood_down = (await teamMemberIds(supabase, before.team_id)).filter(
        (x) => x !== user.id && !newlyAssigned.has(x)
      );
      if (stood_down.length) {
        await notifyMany(supabase, stood_down, {
          task_id: id,
          type: "updated",
          title: "Task reassigned",
          message: `${task.title} was given to another team`,
        });
      }
    }
    ...
    if (changes.status && isFinished(task.status) && !isFinished(before.status)) {
      ...
      // team_id is excluded for the same reason as assigned_to: whoever the
      // work moved to or from has already been told directly above, and this
      // would tell some of them a second time.
      const meaningful = Object.keys(changes).filter(
        (k) => !["assigned_to", "project_id", "team_id", "remind_at"].includes(k)
      );
    ...
    const [{ count }, myProjects, myTeams, reminders] = await Promise.all([
      supabase.from("subtasks").select("id", { count: "exact", head: true }).eq("task_id", id),
      userProjectIds(supabase, user.id),
      userTeamIds(supabase, user.id),
    ...
    return NextResponse.json({
      task: { ...task, remind_at: shownReminder, subtask_count: count || 0, for_me: isForMe(task, user.id, myProjects, myTeams) },
    });
  } catch (error: any) {
