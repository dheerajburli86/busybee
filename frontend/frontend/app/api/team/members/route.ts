...
export async function GET(request: NextRequest) {
  try {
    ...
    // SOW #41: which department each person sits in. Asked for separately
    // because `department_id` only exists after the teams migration - naming
    // it in the select above would fail the whole request (and empty the
    // people list) on a database that hasn't had that migration run.
    const departmentOf = new Map<string, string | null>();
    try {
      ...
      if (!deptErr) {
        (depts || []).forEach((d: any) => {
          if (d.department_id) departmentOf.set(d.user_id, d.department_id);
        });
      }
    } catch {
      /* departments aren't set up yet */
    }
    ...
    const members = data?.map((dm: any) => ({
      ...
      department_id: departmentOf.get(dm.user_id) ?? null,
    })) || [];
