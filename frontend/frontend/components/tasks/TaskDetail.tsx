...
export function TaskDetail({
  ...
  // SOW #41/#17: teams to hand work to, and the project's real milestones.
  // Both arrived after the rest of the app, so a database without the latest
  // migration simply leaves these empty and the panel behaves as before.
  const [teams, setTeams] = useState<Team[]>(lookups.teams || []);
  const [milestones, setMilestones] = useState<Milestone[]>(lookups.milestones || []);
  ...
  /**
   * SOW #41/#17: teams to hand the work to, and the real milestones of this
   * task's project. Both are optional extras - on a database that hasn't had
   * the teams/milestones migration run these requests fail, and the panel
   * just carries on without the two pickers.
   */
  useEffect(() => {
    if (lookups.teams) return; // the page already loaded them for us
    let live = true;
    (async () => {
      try {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const r = await fetch(`/api/milestones?project_id=${task.project_id}`, { cache: "no-store" });
        // Clear either way: leaving the previous project's milestones in the
        // picker would offer a milestone this task may not be pointed at.
        if (!r.ok) {
          if (live) setMilestones([]);
          return;
        }
        const d = await r.json();
        if (live) setMilestones(d.milestones || []);
      } catch {
        if (live) setMilestones([]);
      }
    })();
            ...
            <>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
                ...
                <label className="flex flex-col gap-1">
                  ...
                  {milestones.length > 0 ? (
                    // SOW #17: real milestones belonging to this project, each
                    // with a date of its own.
                    <>
                      <select
                      ...
                      {task.milestone && !task.milestone_id && (
                        <span className="text-[11px] text-slate-500">
                          Was: {MILESTONES.find((m) => m.value === task.milestone)?.label ?? task.milestone}
                        </span>
                      )}
                    </>
                  ) : (
                    // No milestones defined on this project yet (or the database
                    // update hasn't been run): the original fixed list.
              ...
              <div className={section}>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
                  ...
                  {teams.length > 0 && (
                    // SOW #29/#41: the work can go to a whole team or an
                    // ad-hoc group as well as to one person. Everyone in it
                    // sees the task and is told when it changes.
                    <label className="flex flex-col gap-1">
                      <span className="text-slate-400 text-xs">Team or group</span>
                      <select
                        value={task.team_id || ""}
                        disabled={!canManage}
                      ...
                      >
                        <option value="">Nobody</option>
                        {teams.some((t) => t.kind !== "group") && (
                          <optgroup label="Teams">
                            {teams.filter((t) => t.kind !== "group").map((t) => (
